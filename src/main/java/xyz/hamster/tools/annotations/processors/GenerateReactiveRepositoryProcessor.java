...
@SupportedAnnotationTypes("xyz.hamster.tools.annotations.GenerateReactiveRepository")
@SupportedSourceVersion(SourceVersion.RELEASE_11)
@AutoService(Processor.class)
public class GenerateReactiveRepositoryProcessor extends AnnotationProcessor {
        ...
        for (TypeElement annotation : annotations) {
            for (Element element : roundEnv.getElementsAnnotatedWith(annotation)) {
                try {
                    ...
                    TypeName entityIdType = EntityIdType.valueOf(element.getAnnotation(GenerateReactiveRepository.class).id().toString()).getType();
                    TypeName collectionType = ParameterizedTypeName.get(ClassName.get("reactor.core.publisher", "Flux"), entityType);
                    TypeName crudInterface = ParameterizedTypeName.get(ClassName.get("org.springframework.data.repository.reactive", "ReactiveCrudRepository"), entityType, entityIdType);
                    ...
                    if (Boolean.TRUE.equals(element.getAnnotation(GenerateReactiveRepository.class).hasActiveFlag())) {
    ...
    private String getDestinationPackage(Element element) {
        String packageName = element.getAnnotation(GenerateReactiveRepository.class).destinationPackage();
